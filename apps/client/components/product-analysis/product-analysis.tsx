...
import { Calculator, ChevronLeft, ClipboardList, CreditCard, Download, HelpingHand, Search, ShoppingCart, User } from "lucide-react";
import { useQuery } from "@tanstack/react-query";
import { CatalogType, StoreType } from "@/lib/types";
import { getCatalogs } from "@/query/queryFn";
...
export function ProductAnalysis({ storeId, stores }: Props) {
  ...
  const getOrderedItems = (selectedData: CatalogType[], orderQuantities: Record<number, number>, formState: OrderFormState) => {
    ...
    if (formState.prepRequired && formState.prepRequired.toLowerCase() !== 'no') {
      ...
      const prepFeeItem: CatalogType & { quantity: number; line_total: number } = {
        id: -1,
    ...
    if (formState.paymentMethod === 'credit_card') {
      ...
      const creditCardFeeItem: CatalogType & { quantity: number; line_total: number } = {
        id: -2,
        store_id: Number(storeId),
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute('download', `CatalistGroup_catalog_${new Date().toISOString().slice(0, 10)}.csv`);
    link.click();
  };

  return (
    <div>
      <h2 className="text-xl font-semibold mb-2">{!showOrderPanel ? "Master Catalogue" : "Create Purchase Order"}</h2>
      {showOrderPanel &&
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm text-gray-500">Please make sure you read our FAQ in entirety before completing an order form.</div>
          <div onClick={() => { setSelected([]); setShowOrderPanel(false); }} className="min-w-28 text-blue-600 cursor-pointer flex items-center gap-1 hover:text-blue-800 transition-colors">
            <ChevronLeft className="h-4 w-4" />Go Back
          </div>
        </div>}

      {!showOrderPanel && <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-8 gap-4">
        <MetricCard
        ...
        {showOrderPanel ? (
              ...
              <div className="space-y-6">
                <div className="bg-[#f8fafc] rounded-xl border border-[#e5e7eb] p-6 space-y-6">
                  <div className="flex items-center gap-2 mb-2">
                    <ClipboardList />
                    <h3 className="text-base font-semibold">Preferences <span className="text-red-600">*</span></h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setFormState((s: OrderFormState) => ({
                          ...s,
                          paymentMethod: v || ""
                        }))
                        }>
                        <SelectTrigger className={`w-full bg-white border ${errors.paymentMethod ? 'border-red-500' : 'border-[#e5e7eb]'
                          } rounded-lg`}>
                    ...
                    <div>
                      <label className="block text-sm font-medium mb-1">Prep Required <span className="text-red-600">*</span></label>
                      <Select
                        ...
                        <SelectTrigger className={`w-full bg-white border ${errors.prepRequired ? 'border-red-500' : 'border-[#e5e7eb]'
                          } rounded-lg`}>
                          <SelectValue placeholder="Select" />
                        </SelectTrigger>
                        <SelectContent>
                      ...
                      {errors.prepRequired && <p className="text-red-500 text-xs mt-1">{errors.prepRequired}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#f8fafc] rounded-xl border border-[#e5e7eb] p-6 space-y-6">
                  <div className="flex items-center gap-2 mb-2">
                    <HelpingHand />
                    <h3 className="text-base font-semibold">Ungating Assistance <span className="text-red-600 font-semibold">(Mandatory)</span></h3>
                  </div>
                  <ol className="list-decimal text-sm text-gray-600 pl-5 space-y-1">
                    <li>After submitting this form, send a user permissions invitation to <span className="font-medium">ungate@catalistgroup.co</span></li>
                    <li>Our team will accept the invitation.</li>
                    <li>Once accepted please grant View/Edit access for Manage Inventory, Add a Product, Manage Selling Applications, Manage Case Log.</li>
                  </ol>
                  <div className="flex items-center gap-6 mt-2">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={() => setFormState((s: OrderFormState) => ({ ...s, ungateAssistance: "yes" }))}
                      />
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={() => setFormState((s: OrderFormState) => ({ ...s, ungateAssistance: "no" }))}
                      />
                      <span className="text-sm">No</span>
                    </label>
                    {errors.ungateAssistance && <p className="text-red-500 text-xs mt-1">{errors.ungateAssistance}</p>}
                  </div>
                </div>
                ...
                <div className="bg-[#f8fafc] rounded-xl border border-[#e5e7eb] p-6 space-y-6">
                  <div className="flex items-center gap-2 mb-2">
                    <User />
                    <h3 className="text-base font-semibold">Client Details <span className="text-red-600">*</span></h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1">First Name <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter first name" value={formState.firstName} onChange={(e) => setFormState((s: OrderFormState) => ({ ...s, firstName: e.target.value }))} />
                      {errors.firstName && <p className="text-red-500 text-xs mt-1">{errors.firstName}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Last Name <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter last name" value={formState.lastName} onChange={(e) => setFormState((s: OrderFormState) => ({ ...s, lastName: e.target.value }))} />
                      {errors.lastName && <p className="text-red-500 text-xs mt-1">{errors.lastName}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Email <span className="text-red-600">*</span></label>
                      <Input type="email" placeholder="Enter email address" value={formState.email} onChange={(e) => setFormState((s: OrderFormState) => ({ ...s, email: e.target.value }))} />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Input type="tel" placeholder="Enter phone number" value={formState.phone} onChange={(e) => setFormState((s: OrderFormState) => ({ ...s, phone: e.target.value }))} className="flex-1" />
                      </div>
                      {errors.phone && <p className="text-red-500 text-xs mt-1">{errors.phone}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Company <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter company name" value={formState.company} onChange={(e) => setFormState((s: OrderFormState) => ({ ...s, company: e.target.value }))} />
                      {errors.company && <p className="text-red-500 text-xs mt-1">{errors.company}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Marketplace <span className="text-red-600">*</span></label>
                      <Select
                        value={formState.storefront}
                        onValueChange={(v) => setFormState((s: OrderFormState) => ({ ...s, storefront: v }))}
                        defaultValue="Amazon"
                      >
                      ...
                      {errors.storefront && <p className="text-red-500 text-xs mt-1">{errors.storefront}</p>}
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-[#f8fafc] rounded-xl border border-[#e5e7eb] p-6 space-y-6">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="text-base font-semibold">Billing Address <span className="text-red-600">*</span></h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium mb-1">Street <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter street address" value={formState.street} onChange={e => setFormState((s: OrderFormState) => ({ ...s, street: e.target.value }))} />
                      {errors.street && <p className="text-red-500 text-xs mt-1">{errors.street}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">City/Suburb</label>
                      <Input placeholder="Enter city/suburb" value={formState.city} onChange={e => setFormState((s: OrderFormState) => ({ ...s, city: e.target.value }))} />
                      {errors.city && <p className="text-red-500 text-xs mt-1">{errors.city}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Zip/Post Code <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter ZIP code" value={formState.zip} onChange={e => setFormState((s: OrderFormState) => ({ ...s, zip: e.target.value }))} />
                      {errors.zip && <p className="text-red-500 text-xs mt-1">{errors.zip}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">State <span className="text-red-600">*</span></label>
                      <Input placeholder="Enter state" value={formState.state} onChange={e => setFormState((s: OrderFormState) => ({ ...s, state: e.target.value }))} />
                      {errors.state && <p className="text-red-500 text-xs mt-1">{errors.state}</p>}
                    </div>
                    <div>
                      <label className="block text-sm font-medium mb-1">Country <span className="text-red-600">*</span></label>
                      <Select value={formState.billingCountry} onValueChange={v => setFormState((s: OrderFormState) => ({ ...s, billingCountry: v }))}>
                        <SelectTrigger className="w-full bg-white border border-[#e5e7eb] rounded-lg">
                ...
                <div className="flex justify-end">
                  <Button onClick={handleCreateOrder} disabled={isCreatingOrder} className="rounded-md px-8">{isCreatingOrder ? "Processing..." : "Submit Order"}</Button>
                </div>
              </div>
          ...
          <Card className="border-none shadow-md">
            <CardContent className="pb-0">
              ...
              <div className={`flex flex-col md:flex-row gap-y-5 mt-10 ${rootData.user?.type == "store" ? "" : "pt-8"} items-center justify-between`}>
            ...
            <CardContent className="py-4 px-6">
              {/* <Tabs
                ...
                className="w-full"
              >
                <div className="px-4 sm:px-0 flex gap-3 mb-4">
                  <TabsList className=" w-fit overflow-x-auto flex-nowrap justify-start">
                    <TabsTrigger value="all" className="px-10">All</TabsTrigger>
                    <TabsTrigger value="latest" className="px-10">Latest</TabsTrigger>
                  </TabsList>
                  <div className="w-fit">
                    <Select>
                      <SelectTrigger>
                        <SelectValue placeholder="Catalist Group" className="w-full" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="catalist_group">Catalist Group</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
      ...
      {showOrderPanel && <div className="flex flex-col md:flex-row gap-4 mt-8">
        <div className="flex-1 bg-white rounded-xl border border-[#e5e7eb] p-6 flex items-start gap-3">
          <span className="mt-1 text-blue-500">
            <svg width="24" height="24" fill="none"><rect x="4" y="4" width="16" height="16" rx="3" stroke="#2563EB" strokeWidth="2" /><path d="M8 8h8M8 12h8M8 16h4" stroke="#2563EB" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
          </span>
          <div>
            <div className="font-semibold text-base mb-1">Terms and Conditions</div>
            <div className="text-sm text-gray-600">
              By submitting this order sheet, you agree to abide by the terms and conditions outlined by Catalist Group LLC.
            </div>
          </div>
        </div>
        <div className="flex-1 bg-white rounded-xl border border-[#e5e7eb] p-6 flex items-start gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-gray-600">
              Catalist Group LLC &bull; <a href="mailto:sales@catalistgroup.co" className="underline">sales@catalistgroup.co</a> &bull; 287 Park Avenue South New York, NY 10010
            </div>
          </div>
        </div>
      </div>}
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="space-y-2">
                <p className="font-medium">The following items have quantities less than their Minimum Order Quantity (MOQ):</p>
                  ...
                  <ul className="list-disc font-bold pl-4 space-y-3">
                    {lowQuantityItems.map((item, index) => (
                      <li
                        ...
                        title={item.name}
                      >
                        <span className="font-mono text-xs bg-gray-100 px-2 py-0.5 rounded">{item.asin}</span>
                        <span className="truncate max-w-[180px] font-medium">{item.name?.length > 80 ? item.name.slice(0, 80) + "..." : item.name}</span>
                ...
                <p className="font-medium mt-4">
                  This order needs to be approved by one of our team members. Do you want to proceed with the order anyway?
                </p>
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
